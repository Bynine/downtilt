...
public abstract class Fighter extends Hittable{
	...
	public void update(List<Rectangle> rectangleList, List<Entity> entityList, int deltaTime){
		...
		if (isTrembling()){
			if (DowntiltEngine.getDeltaTime() % 2 == 0 && !doesCollide(position.x + 2, position.y)) position.set(position.x + 2, position.y);
	...
	public void ground(){
		super.ground();
		if (hitstunTimer.getCounter() > 1 && velocity.y < 0) checkFallOnGround();
		tumbling = false;
	...
	public boolean canBlock() { return canMove() && !inHitstun() && state != State.FALLEN && state != State.HELPLESS; }
