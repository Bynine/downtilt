...
public class VictoryScreen extends Menu{
	...
	private static void draw(){
		...
		batch.begin();
		if (currentVictory.getTime() != Victory.NOTUSED) font.draw(batch, "TIME: " + GlobalRepo.getTimeString(currentVictory.getTime()), startX, startY -= dec);
		...
		//font.draw(batch, "TOTAL: " + currentVictory.getScore(), startX, startY -= dec);
		specialFont.draw(batch, "RANK: " + currentVictory.getRanking(), startX, startY -= dec * 2);
		font.draw(batch, "Press Y to continue", startX, startY -= dec);
		batch.end();
	}
